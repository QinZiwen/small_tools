...
struct SubscribeStub {
    ...
    inline operator Priority() const {
        return priority;
    }  // for comparation : <, ==, >
...
class Message {
  ...
  public:
    Message& on(Executor::Id executor) {
        _subscribe.executor = executor;
        return *this;
    }
    Message& token(const Token& token) {
        _subscribe.token = token;
        return *this;
    }
    Message& priority(Priority priority) {
        _subscribe.priority = priority;
        return *this;
    }
...
template <typename T>
void publish(T& t, Executor::Id executor = Executor::Sync) {
    SubscribeStub::publish(t, {}, executor);
}

template <typename T>
void publish(T& t, const Token& token, Executor::Id executor = Executor::Sync) {
