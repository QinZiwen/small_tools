...
class FunctionWrapper {
  public:
    ...
    static std::unique_ptr<FunctionWrapper> makeFunctionWrapper(Func&& f) {
        return std::make_unique<FunctionWrapperImpl<std::decay_t<Func>>>(std::forward<Func>(f));
    }
};
...
class FunctionWrapperImpl : public FunctionWrapper {
  ...
  public:
    ...
    ~FunctionWrapperImpl() {
        std::cout << "dectory FunctionWrapperImpl" << std::endl;
    }
...
int main() {
    ...
    // 添加不同类型的函数
    functions.push_back(
        FunctionWrapper::makeFunctionWrapper([]() { std::cout << "Lambda with no arguments" << std::endl; }));

    functions.push_back(FunctionWrapper::makeFunctionWrapper(
        [value = 42]() { std::cout << "Lambda with captured value: " << value << std::endl; }));
